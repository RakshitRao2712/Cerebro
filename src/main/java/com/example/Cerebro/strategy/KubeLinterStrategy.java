...
@Component
public class KubeLinterStrategy implements DevOpsLinterStrategy {

    @Override
    public EvalResult validate(Path workspace) {
        try {
            String output = CommandRunner.runCommand(workspace, "kube-linter", "lint", "deployment.yaml");
